...
/*Görev 1: macSkoru()
  ...
  Skor1 ve Skor2 arasındaki temel fark, Skor1'in bir closure kullanmasıdır. skor1 bir fonksiyon döndürürken, 
  Skor2 doğrudan bir fonksiyon olarak tanımlanmıştır.
  
  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
  Skor1 bir closure kullanmaktadır. Closure, bir iç fonksiyonun, dış fonksiyonun kapsamındaki değişkenlere erişim 
  sağlaması durumudur. skorArtirici() fonksiyonu, içinde tanımlanan skor değişkenine iç fonksiyon skorGuncelle() 
  tarafından erişim sağlar. Bu nedenle, skor1 bir closure örneğidir. Closure, iç fonksiyonun dış fonksiyonunun 
  kapsamındaki değişkenleri "kapsar" ve sonradan bu değişkenlere erişebilir.
  
  3. Hangi durumda skor1 tercih edilebilir? Hangi durumda skor2 daha mantıklıdır?
  Skor1, skor değişkenini koruma altına alarak, skoru sadece skorGuncelle() fonksiyonu aracılığıyla güncelleyebilir. 
  Bu durumda, skor1 tercih edilebilir çünkü dışarıdan skor değişkenine erişim kısıtlanır ve sadece belirlenen fonksiyon 
...
const skorlar = periyotSkoru(takimSkoru);
console.log(skorlar);
...
function skorTabelasi(periyotSkoruCallBack, takimSkoruCallBack, ceyrekSayisi) {
  ...
  for (let i = 0; i < ceyrekSayisi; i++) {
    ...
    sonuc.push(
      `${i + 1}. Periyot: Ev Sahibi ${periyot.EvSahibi} - Konuk Takim ${
        periyot.KonukTakim
      }`
    );
  ...
  if (evSahibiSkor === konukTakimSkor) {
    ...
    while (evSahibiSkor === konukTakimSkor) {
      ...
      const periyot = periyotSkoruCallBack(takimSkoruCallBack);
      sonuc.push(
        `${i}. Uzatma: Ev Sahibi ${periyot.EvSahibi} - Konuk Takim ${periyot.KonukTakim}`
      );
      evSahibiSkor += periyot.EvSahibi;
      konukTakimSkor += periyot.KonukTakim;
    }
  ...
  sonuc.push(
    `Maç Sonucu: Ev Sahibi ${evSahibiSkor} - Konuk Takim ${konukTakimSkor}`
  );
